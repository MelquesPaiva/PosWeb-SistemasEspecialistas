...
CAMINHO_ARQUIVOS = os.getcwd() + "/chat/static/arquivos"
MODO_DEFAULT = "chaves"
...
def pesquisar_artigos(chaves, modo):
    artigos_selecionados = []
    sucesso, resposta = acessar_robo(URL_ROBO_PESQUISAR_ARTIGOS, {"criterios": chaves, "modo": modo})
    if sucesso:
...
def get_resposta():
    ...
    return Response(
        json.dumps({"resposta": resposta, "artigos": artigos, "artigos_pesquisados": pesquisar_por_artigos}),
        status=200,
