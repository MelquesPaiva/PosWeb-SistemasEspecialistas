...
def get_artigos():
    conteudo = request.json
    chaves, artigos_selecionados, encontrou = [], {}, False
    modo = conteudo["modo"]
    criterios = conteudo['criterios'] if 'criterios' in conteudo else []

    if modo == "chaves":
        encontrou, artigos_selecionados = pesquisar_artigos_por_chaves(criterios, artigos)

    elif modo == "areas":
        encontrou, artigos_selecionados = pesquisar_artigos_por_areas(criterios, artigos)

    return Response(
        json.dumps({"artigos": list(artigos_selecionados.values())}),
